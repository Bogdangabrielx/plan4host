  ...
  try {
    ...
    const topicLabel =
      topic === "wifi"
        ...
        : topic === "minibar"
        ? "Minibar: whether there is a minibar, where it is, whether items inside are complimentary or paid extra, and, if prices are explicitly listed, the exact price for each item in the same currency and format written by the host. You may translate the product names and descriptions into the target language, but you must keep all numbers and currency tokens exactly as written (for example \"15 lei\", \"5 EUR\", \"$3\")."
...
- For minibar, if the text lists concrete products with prices, mention each product together with its exact price and currency exactly as written (for example "5 EUR", "10 lei", "$3"), without converting or changing any numbers or currency symbols. You MAY translate the product names and descriptions into the target language, but you MUST keep numbers and currency tokens exactly as they appear, and you MUST NOT invent or infer any product or price that is not explicitly listed. If only a generic statement like "paid extra" is present but no exact prices, mention that it is paid extra but do NOT invent prices.
