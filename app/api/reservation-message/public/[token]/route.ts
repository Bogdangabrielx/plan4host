...
export async function GET(req: NextRequest, ctx: { params: { token: string } }) {
  try {
    ...
    try {
      const accId = (prop as any)?.account_id as string | null;
      if (accId) {
        const rAcc = await admin.from('accounts').select('plan').eq('id', accId).maybeSingle();
        if (!rAcc.error && rAcc.data) {
          const plan = String((rAcc.data as any).plan || '').toLowerCase();
          guestAiEnabled = plan === 'premium';
        }
      }
    } catch {
