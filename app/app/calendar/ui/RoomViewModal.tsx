...
export default function RoomViewModal({
  ...
  // Exact integration logo by integration id
  const integLogoById = useMemo(() => {
    const m = new Map<string, string>();
    for (const it of integrations) {
      if (!it?.is_active) continue;
      if ((it as any).id && it.logo_url) {
        m.set(String((it as any).id), it.logo_url);
      }
    }
    return m;
  }, [integrations]);
  ...
  const integLogoByRoomId = useMemo(() => {
    ...
    for (const it of integrations) {
      if (!it?.is_active) continue;
      if (it.room_id && it.logo_url) {
        if (!m.has(it.room_id)) m.set(it.room_id, it.logo_url);
      }
    }
    return m;
  }, [integrations]);
  ...
  const integLogoByTypeId = useMemo(() => {
    ...
    for (const it of integrations) {
      if (!it?.is_active) continue;
      if (it.room_type_id && it.logo_url) {
        if (!m.has(it.room_type_id)) m.set(it.room_type_id, it.logo_url);
      }
    }
    return m;
  }, [integrations]);

  // Provider logo by provider slug (fallback when integration-specific missing)
  const providerLogos = useMemo(() => {
    const m = new Map<string, string>();
    for (const it of integrations) {
      if (!it?.is_active) continue;
      const p = normalizeProvider(it.provider);
      if (it.logo_url && !m.has(p)) m.set(p, it.logo_url);
    }
    return m;
  }, [integrations]);
  ...
  const logoByRoomDate = useMemo(() => {
    ...
    for (const b of bookings) {
      ...
      // 1) Integration-specific logo
      const intId = (b as any).ota_integration_id as string | undefined;
      if (intId && integLogoById.has(intId)) {
        logo = integLogoById.get(intId);
      }
      if (key !== "manual") {
        // 2) Provider default logo
        if (!logo && key !== "other") logo = providerLogos.get(key);
        // 3) Integration logo by room or type
        if (!logo) {
          logo = (b.room_id && integLogoByRoomId.get(b.room_id))
    ...
    return map;
  }, [bookings, providerLogos, integLogoByRoomId, integLogoByTypeId, integLogoById]);
      ...
      {cells.map((c, i) => {
        ...
        return (
          ...
          >
            ...
            {logoUrl && (
              <div style={{ position:'absolute', right:4, bottom:4, width:16, height:16, borderRadius:4, background:'rgba(255,255,255,0.9)', display:'grid', placeItems:'center', boxShadow:'0 0 0 1px rgba(0,0,0,0.08)' }}>
                <img src={logoUrl} alt="OTA" style={{ width:'90%', height:'90%', objectFit:'contain', display:'block' }} />
              </div>
            )}
