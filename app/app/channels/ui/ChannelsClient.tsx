      ...
      <section className="sb-card" style={{ padding: 16, marginTop: 8 }}>
        ...
        <div style={{ display: "grid", gap: 12 }}>
          ...
          >
            <div
              className="sb-card"
              style={{ display: 'grid', gap: 10, padding: 12, width: isSmall ? '100%' : 280, ...(isSmall ? {} : { aspectRatio: '5 / 4' }) }}
            >
...
function ManageRoomModal({
  ...
  // Logo map per room (for "Other")
  const LS_LOGO_KEY = `p4h:otaLogos:room:${room.id}`;
  const [logoMap, setLogoMap] = useState<Record<string, string>>({});
  useEffect(() => {
    try {
      const raw = localStorage.getItem(LS_LOGO_KEY);
  ...
  // Single hidden input for picking logos
  const fileInputRef = useRef<HTMLInputElement | null>(null);
  const logoTargetProviderRef = useRef<string>("other");
  const logoTargetIntegrationIdRef = useRef<string | null>(null);
  async function validateAndReadPNG(file: File): Promise<string> {
  ...
  async function persistColor(integrationId: string, c: string) {
    try {
      const { data, error } = await supa
        ...
        .select("id,color,logo_url")
        .single();
      if (!error && data) onUpdate(integrationId, { color: (data as any).color ?? c });
    } catch { /* noop */ }
  }
            ...
            <button className="sb-btn sb-btn--primary" onClick={async () => {
              ...
              if (newId) {
                ...
                if (provider === 'Other') {
                  ...
                  if (dataUrl) {
                    try {
                      const res = await fetch('/api/ical/logo/upload', { method: 'POST', body: (()=>{ const fd=new FormData(); fd.append('integrationId', newId); const m=dataUrl.match(/^data:([^;]+);base64,/); const mime=(m?m[1]:'image/png'); const bin=atob(dataUrl.split(',')[1]||''); const ab=new ArrayBuffer(bin.length); const ia=new Uint8Array(ab); for(let i=0;i<bin.length;i++) ia[i]=bin.charCodeAt(i); const file=new File([ab], 'logo.png', { type: mime }); fd.append('file', file); return fd; })() });
                      const j = await res.json().catch(()=>({}));
                      if (res.ok && j?.url) onUpdate(newId, { logo_url: j.url });
                    } catch {}
                  }
