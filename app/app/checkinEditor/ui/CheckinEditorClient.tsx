...
export default function CheckinEditorClient({ initialProperties }: { initialProperties: Array<{ id: string; name: string }> }) {
  ...
  // Placeholder presentation image reminder
        ...
        <div
          ...
          <div onClick={(e)=>e.stopPropagation()} className="sb-card" style={{ width:'min(540px, 100%)', background:'var(--panel)', border:'1px solid var(--border)', borderRadius:12, padding:16, display:'grid', gap:12, position:'relative' }}>
            <button
              ...
              style={{ position:'absolute', top:12, right:12, width:28, height:28, borderRadius:999, border:'1px solid var(--border)', background:'var(--card)', cursor:'pointer' }}
            >
              ×
            </button>
            ...
            <div style={{ color:'var(--text)', display:'grid', gap:8 }}>
              <div>Your current photo is a placeholder. Add your real property photo so guests recognize the place in the custom check-in form.</div>
              <ul style={{ margin:0, paddingLeft:18, color:'var(--muted)', display:'grid', gap:6 }}>
                <li>The image appears on the personalized check-in form.</li>
                <li>With contact details filled in, guests can reach you easily from automated/programmed messages.</li>
              </ul>
            </div>
            <div style={{ display:'flex', justifyContent:'flex-end', gap:8, flexWrap:'wrap' }}>
              <button
              ...
              <button
                ...
                onClick={() => {
                  ...
                  setTimeout(() => { try { triggerImageUpload(); } catch {} }, 60);
                }}
              >
                Upload now
              </button>
