...
-- Changelog:
--   2025-09-13
--     - Canonical tenant key = properties.admin_id (no owner_id)
--     - Single source of truth for plan = accounts.plan (account_plan removed)
...
/* =====================================================================
   ...
   - public.account_can_sync_now_v2(p_account_id uuid, p_event_type text) -> jsonb
       ...
       Deterministic staggering on 5-minute rounds to avoid spikes:
         - Round duration: 5 minutes; round_index = floor(epoch_seconds/300)
         - Premium  → slot_mod=2 (effective 10m)
           Standard → slot_mod=6 (effective 30m)
           Basic    → slot_mod=12 (effective 60m)
         - Slot selection: slot_idx = abs(hashtextextended(account_id::text,0)) % slot_mod
           Allowed only if (round_index % slot_mod) == slot_idx
       Uses public.account_sync_usage to compute cooldown and returns JSON:
         { allowed:boolean,
           ...
           remaining_in_window:int|null,  -- 5-minute rounds until next slot
...
/* =====================================================================
  4) TRIGGERS (ACTIVE)
   ---------------------------------------------------------------------
   Team — Premium only (via RLS)
   - Enforced by RLS policies on public.account_users:
     admin-only + accounts.plan='premium' for INSERT/UPDATE/DELETE; self SELECT allowed.

   Cleaning — not available on Basic (via RLS)
   - Enforced by RLS policy p_cp_write_scope_plan on public.cleaning_progress:
     writes allowed only when accounts.plan IN ('standard','premium').

   Trial auto-grant (admin account bootstrap, OAuth/signup)
   ...
   - auth.users → public.handle_new_user() AFTER INSERT (trigger name: on_auth_user_created)
     ...
     - Skips tenant bootstrap when NEW.raw_app_meta_data/user_meta_data has sub_user=true
...
/* =====================================================================
 ...
 ORDER BY a.created_at;
   ...
   -- b) Check essential onboarding trigger attached (Auth)
   SELECT event_object_table, trigger_name, action_timing, event_manipulation
     FROM information_schema.triggers
